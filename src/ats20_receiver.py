...
class HardwareDataCollector:
    ...
    def initialize_sdr(self, center_freq: float = 100e6, sample_rate: float = 2.4e6,
        ...
        """Initialize RTL-SDR device."""
        if not RTLSDR_AVAILABLE:
            logger.error("RTL-SDR library not available. Install with: pip install rtlsdr")
            return False
    ...
    def sdr_to_frequency_domain(self, sdr_data: np.ndarray) -> pd.DataFrame:
        ...
        # Apply FFT to IQ samples
        fft = np.fft.fft(sdr_data)
        freqs = np.fft.fftfreq(len(sdr_data), 1/self.sdr.sample_rate)
    ...
    def list_sdr_devices(self) -> List[Dict]:
        ...
        try:
            # RTL-SDR library doesn't provide device listing easily
            # Try to initialize to test
            test_sdr = RtlSdr()
            test_sdr.close()
            return [{'index': 0, 'name': 'RTL-SDR Device'}]
        except:
            return []
...
def main():
    ...
    if audio_devices:
        ...
        if collector.initialize_audio():
            ...
            print("✗ Audio device initialization failed")
    else:
    ...
    # Collect sample data
    if collector.audio or collector.sdr:
        print("\nCollecting sample data...")
        data = collector.collect_hardware_data(1.0, 1.0)
